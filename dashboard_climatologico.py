...
# Mapa Interactivo
    ...
    st.write("Explora patrones climáticos alrededor del mundo con este mapa interactivo. Utiliza los filtros disponibles para visualizar datos específicos según el mes y la variable seleccionada, ya sea precipitaciones o temperaturas.")
    ...
    if not df_filtrado.empty:
        # Asegurar que los valores de la columna seleccionada sean positivos
        if filtro_variable == 'Temperatura':
            df_filtrado[filtro_variable] = df_filtrado[filtro_variable] - df_filtrado[filtro_variable].min() + 1
        elif filtro_variable == 'Precipitaciones':
            df_filtrado[filtro_variable] = df_filtrado[filtro_variable].apply(lambda x: x if x > 0 else 0.1)  # Evitar valores negativos o 0
        ...
        fig_mapa = px.scatter_mapbox(
            ...
            title=f"Mapa Interactivo de {filtro_variable} en {filtro_mes}",
